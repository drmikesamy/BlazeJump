...
namespace BlazeJump.Common.Pages.Components
{
	public partial class MessageCard
	{
		...
		public NMessage TaggedReply => MessageService.RelationRegister.TryGetRelations(new List<string> { NEvent.Id }, FetchTypeEnum.TaggedParentIds, out var replies) ? replies.Select(id => MessageService.MessageStore[id]).FirstOrDefault() : null;
		public int TaggedReplyCount => MessageService.RelationRegister.TryGetRelations(new List<string> { NEvent.Id }, FetchTypeEnum.TaggedParentIds, out var replies) ? replies.Select(id => MessageService.MessageStore[id]).Count() : 0;
		public NMessage TaggedRoot => MessageService.RelationRegister.TryGetRelations(new List<string> { NEvent.Id }, FetchTypeEnum.TaggedRootId, out var replies) ? replies.Select(id => MessageService.MessageStore[id]).FirstOrDefault() : null;
