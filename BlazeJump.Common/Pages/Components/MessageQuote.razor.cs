...
namespace BlazeJump.Common.Pages.Components
{
    public partial class MessageQuote
    {
        ...
        public NMessage TaggedReply => MessageService.RelationRegister.TryGetRelations(new List<string> { NEvent.Id }, RelationTypeEnum.EventIdToReplyEventId, out var replies) ? replies.Select(id => MessageService.MessageStore.ContainsKey(id) ? MessageService.MessageStore[id] : new NMessage()).FirstOrDefault() : null;
